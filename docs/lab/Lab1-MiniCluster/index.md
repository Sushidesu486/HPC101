...
!!! tip "如何写一份好的实验报告"
    ...
    2. 详略得当。一般来说，下面这几种实验报告都不是好的实验报告：
        ...
        - 长段的ai生成内容： 大段大段的ai废话，看不到学生的思考过程。
    3. 关于AI内容：
...
解决这个问题的一种朴素方法是手动把同一份目录复制到每个节点。节点数量一朵，我们不可能直接复制每一个文件到每一个节点上但真实集群通常不会这样管理用户文件，而是使用**共享文件系统**：多个节点通过网络访问同一套目录树，让可执行文件、输入数据、作业脚本和输出结果出现在一致的位置。对用户来说，它看起来像普通本地目录；对系统来说，读写请求可能会被转发到远端服务器、分布式存储集群或专门的元数据服务。

共享文件系统需要同时解决几个问题：

- **路径一致性**：同一个文件在所有节点上最好出现在同一路径，例如都挂载到 `/cluster/shared`。这样 MPI 程序和 Slurm 作业脚本不用为不同节点写不同路径。
- **数据一致性**：一个节点写入的文件，其他节点应该能看到更新。不同文件系统的一致性模型和缓存策略不同，性能和行为也会不同。
- **权限一致性**：所有节点必须对“谁是文件属主、谁有读写权限”有一致理解。最基础的做法是保证各节点 UID/GID 一致，更完整的集群会配合 LDAP、FreeIPA、NIS 等统一身份服务。
- **性能与扩展性**：小集群可以用简单的单服务器共享目录；规模更大后，元数据压力、并发读写、网络带宽和故障恢复都会成为瓶颈，因此可能会使用 Lustre、CephFS、BeeGFS 等更复杂的并行或分布式文件系统。

本 Lab 选择 NFS（Network File System）作为共享文件系统方案，因为它概念清晰、部署成本低，适合用来理解“一个节点导出目录，其他节点挂载使用”的基本模型。NFS 采用 server / client 结构：服务器负责导出目录，客户端通过网络把远程目录挂载到本地路径。挂载完成后，客户端访问这个目录就像访问本地目录一样，但实际数据仍由 NFS 服务器提供。
...
实际维护集群时，还需要继续关注一些 NFS 配置细节：导出范围不应过大，避免把共享目录暴露给不可信网段；`root_squash`、只读导出、客户端网段限制等选项会影响安全性和可维护性；开机自动挂载需要考虑网络服务是否已经就绪，必要时可以了解 autofs。NFS 适合入门和中小规模共享目录；更大规模集群可能会使用 Lustre、CephFS、BeeGFS 等文件系统。
...
!!! tip "知识点：容器编排和 Kubernetes"
    ...
    如果从**集群运维**角度看，k3s 还可以作为一个很小的“服务平台”来理解：Slurm 负责把计算作业调度到节点上，而 k3s 更适合承载集群周边的长期运行服务，例如 Web 面板、监控组件、日志收集器、内部 API、轻量数据库或网络入口。真实集群里，这些服务不一定全部运行在 Kubernetes 上，但 Kubernetes 提供了一种观察它们如何被部署、发现、扩缩容和恢复的方式。
...
以下任务改为围绕“集群运维服务”展开，且均为带 `*` 的进阶任务，仅作为超算队选拔依据，不参与课程评价。每一项都需要完成实际部署或最小可运行验证，不能只停留在资料调研。
...
- **(\*) 运维故障排查报告**：在你部署的运维服务中构造一个小故障，例如 Pod Pending、Service 访问不到、PVC 绑定失败、节点 NotReady、认证失败或磁盘空间不足。记录你从哪些命令、日志或指标开始排查，并说明最终定位到哪一层。
- **(\*) 组合两个以上运维服务**：实际组合两个以上服务，例如 Prometheus + Grafana、Ingress + cert-manager、Rook-Ceph + PVC、FreeIPA/LDAP + NFS 权限模型，说明它们如何协同支撑一个真实集群。
...
??? 实验秘籍 "如果你实在卡住了……"
    ...
    **前提**：两台机器都已安装  Ubuntu / Debian（或其他主流发行版）、已配好 `/etc/hosts`（互相能 ping 通主机名）、已配置 SSH 免密登录（`node01` → `node02` 不需要密码）。

    如果你走的是 Docker 路线，下面也给出一套**不包含 Bonus** 的本地参考文件。它们对应的是四节点 Docker 集群、NFS、MUNGE、Slurm 和 HPL 的完整实验路径，适合作为“实在卡住时”的对照版本。

    **注意**： 如果你参考了我们的 ~~焚决~~ 脚本，请在报告中说明，并且在报告中写出你对脚本在干什么的理解。例如:这些脚本帮你完成了什么事情？脚本运行到什么地方卡住了？如何解决的？等等

    ??? note "参考：Docker Compose 文件"

        ```yaml title="build/lab1-docker/compose.yml"
        services:
          node01:
            build:
              context: ../..
              dockerfile: build/lab1-docker/Dockerfile
    ...
    ??? note "参考：Dockerfile"
        ...
        ```dockerfile title="build/lab1-docker/Dockerfile"
        FROM ubuntu:24.04
    ...
    ??? note "参考：entrypoint.sh"

        ```bash title="build/lab1-docker/entrypoint.sh"
        #!/usr/bin/env bash
        set -euo pipefail
        ...
        for i in "${!ALL_NODES[@]}"; do
            node="${ALL_NODES[$i]}"
            ip="${ALL_IPS[$i]}"
            grep -q "$ip.*$node" /etc/hosts 2>/dev/null || echo "$ip    $node" >>/etc/hosts
        done
        ...
        if [ "${LAB1_NFS_SERVER:-0}" = "1" ]; then
            ...
            grep -q '^/cluster/shared ' /etc/exports 2>/dev/null || echo '/cluster/shared 172.28.0.0/24(rw,sync,no_subtree_check,no_root_squash)' >>/etc/exports
            pidof rpcbind >/dev/null 2>&1 || /sbin/rpcbind 2>/dev/null || rpcbind 2>/dev/null || true
            [ -f /proc/fs/nfsd/threads ] || mount -t nfsd nfsd /proc/fs/nfsd 2>/dev/null || true
            ...
            while [ $retries -lt 15 ]; do
                if mount -t nfs -o nolock,vers=3,proto=tcp,timeo=10,retrans=3 node01:/cluster/shared /cluster/shared 2>/dev/null; then
                    break
                fi
                sleep 3
                retries=$((retries + 1))
        ...
        pidof munged >/dev/null 2>&1 || /usr/sbin/munged 2>/dev/null || true
        ...
        case "$HOSTNAME" in
            node01)
                pidof slurmctld >/dev/null 2>&1 || slurmctld 2>/dev/null || true
                [ -f /etc/munge/munge.key ] && [ ! -f /cluster/shared/munge.key ] && cp /etc/munge/munge.key /cluster/shared/munge.key && chown user:user /cluster/shared/munge.key && chmod 644 /cluster/shared/munge.key || true
                ;;
            *)
                pidof slurmd >/dev/null 2>&1 || slurmd 2>/dev/null || true
                ;;
        esac
    ...
    ??? note "参考：Docker 一键部署脚本"

        ```bash title="build/lab1-docker/setup.sh"
        #!/usr/bin/env bash
        set -euo pipefail
        ...
        for i in $(seq 1 15); do
            ...
            for node in node01 node02 node03 node04; do
                docker exec "hpc101-$node" pgrep sshd >/dev/null 2>&1 && READY=$((READY + 1))
            done
            [ "$READY" -eq 4 ] && break
            sleep 2
        done
        ...
        if [ "$NFS_OK" = true ]; then
            ...
            sleep 1
            docker exec hpc101-node04 bash -c "cat /cluster/shared/.nfs-test 2>/dev/null" | grep -q "NFS shared OK" && log_ok "NFS 跨节点读写验证通过"
            docker exec hpc101-node01 rm -f /cluster/shared/.nfs-test 2>/dev/null || true
        else
            log_warn "未检测到 NFS 挂载，可能是宿主机内核限制"
        fi

        for node in node01 node02 node03 node04; do
            docker exec "hpc101-$node" bash -c "munge -n 2>/dev/null | unmunge 2>/dev/null >/dev/null" && log_ok "MUNGE 正常 ($node)" || log_warn "MUNGE 异常 ($node)"
        done

        docker exec hpc101-node01 bash -c "pidof slurmctld >/dev/null 2>&1 || slurmctld 2>/dev/null"
        for node in node02 node03 node04; do
            docker exec "hpc101-$node" bash -c "pidof slurmd >/dev/null 2>&1 || slurmd 2>/dev/null"
        done
        sleep 2
        docker exec hpc101-node01 sinfo 2>&1 || true
        docker exec hpc101-node01 bash -c "scontrol update NodeName=node[01-04] State=RESUME 2>/dev/null" || true
        docker exec hpc101-node01 srun -N4 hostname 2>&1 || true
        ```

    ---

    将以下内容保存为 `setup_node01.sh`，然后 `sudo bash setup_node01.sh`：
