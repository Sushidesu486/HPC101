...
=== "Docker"
    ...
    Docker 路线下，这一部分的目标与虚拟机路线类似：先把一个稳定的四节点基础集群搭起来，再确认它已经具备后续实验需要的最小能力。可以按下面的思路组织：
    ...
    如果后续还要在容器内运行 NFS、Slurm 等系统服务，最好一开始就启用 `privileged: true`。
    ...
    ??? tip "Docker 集群搭建常见问题"
        ...
        - **容器启动后立即退出**：优先检查 `docker compose logs`。常见原因是 SSH 服务没有以前台方式运行，或者入口脚本提前退出。
        - **容器间无法通过主机名互通**：确认四个容器在同一个自定义网络中；如果使用静态 IP，检查 IP 是否落在同一子网内。
        - **SSH 仍要求输入密码**：通常是镜像里没有提前生成并分发公钥，或者 `~/.ssh`、`authorized_keys` 权限不正确。
        - **`Host key verification failed`**：容器重建后 SSH host key 会变化，可使用 `StrictHostKeyChecking=accept-new`，或清理本地 `known_hosts` 缓存。
        - **MPI 不能跨节点启动**：先不要直接跑 HPL，先用 `hostname` 之类的最小 MPI 命令验证 SSH、主机名解析和可执行文件路径是否都正常。
    ---
    ...
    **报告建议**
    ...
    注意在报告中明确说明你使用的是哪种方案：
    - 如果使用 Docker 路线，应说明你如何构建四节点容器集群，以及如何在 `node01` 上导出 `/cluster/shared`、在 `node02`–`node04` 上完成挂载。
    - 如果使用 VM 路线，则需要说明你如何手动配置 NFS 服务端和客户端。
...
??? success "步骤参考和说明"
    ...
    === "Docker"
        ...
        在 Docker 路线中，建议把 NFS 看成“在已有四节点容器集群上追加的一步”。可以按下面的思路组织：

        - 在 `node01` 上准备 NFS 服务端：安装 `nfs-kernel-server`、`rpcbind` 等依赖，创建共享目录，挂载 `nfsd` 接口，并配置 `/etc/exports` 导出 `/cluster/shared`。
        - 在 `node02`~`node04` 上准备 NFS 客户端：安装 `nfs-common`，创建本地挂载点，并把 `node01:/cluster/shared` 挂载到 `/cluster/shared`。
        - 完成挂载后做一次最小验证：在客户端实际创建一个测试文件，再回到 `node01` 或其他节点确认该文件是否可见，而不要只看 `showmount` 或 `mount` 输出。

        容器环境下，客户端挂载通常建议带上 `nolock` 选项；服务端导出网段则应与你的 Docker 网络保持一致。

        ??? tip "Docker 下 NFS 常见问题"

            - **`exportfs: /cluster/shared does not support NFS export`**：通常是因为还没有执行 `mount -t nfsd nfsd /proc/fs/nfsd`。
            - **`rpc.statd is not running but is required for remote locking`**：客户端挂载时加上 `-o nolock`，这是容器环境下最常见的处理方式。
            - **挂载时报 `access denied`**：检查 `/etc/exports` 里的网段是否覆盖 Docker 网络，例如 `172.28.0.0/24`。
