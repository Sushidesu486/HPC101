...
=== "Docker"
    ...
    - **共享 volume**：用于替代 NFS 实现跨容器文件共享。所有容器挂载同一个 volume 到 `/cluster/shared`。
    ...
    注意在报告中明确说明：**Docker 路线只是用 volume 模拟了共享目录，并没有配置真正的 NFS 服务**。如果使用了 Docker 而跳过 NFS 和 Slurm 的配置，需要在报告中说明原因和你对 NFS / Slurm 工作原理的理解。
...
??? 实验秘籍 "如果你实在卡住了……"
    ...
    **前提**：两台机器都已安装 Ubuntu 24.04、已配好 `/etc/hosts`（互相能 ping 通主机名）、已配置 SSH 免密登录（`node01` → `node02` 不需要密码）。
    ...
    如果你使用的是 Docker 路线而不是虚拟机，可以直接使用仓库中提供的 Docker 参考实现：

    ```bash
    # 进入 docker 配置目录
    cd build/lab1-docker
    ...
    - **`build/lab1-docker/Dockerfile`**：基于 Ubuntu 24.04，内置 OpenMPI 5.0.3、BLAS 3.12.0、CBLAS、HPL 2.3 的编译过程，SSH 密钥自动生成，PATH 已配置好。
    - **`build/lab1-docker/compose.yml`**：定义了 4 个容器（`node01`~`node04`），共享自定义网络 `172.28.0.0/24`，Docker volume `hpc101-shared` 挂载到 `/cluster/shared`。
    - **`build/lab1-docker/entrypoint.sh`**：容器启动时自动配置 SSH、hostname 解析（写入 `/etc/hosts`），以及 rpcbind 和 nfsd 的初始化。

    在容器内配置 NFS + Slurm 时，你需要额外执行：
    ...
    # node02~04 上：挂载 NFS 共享
    mount -t nfs -o nolock node01:/cluster/shared /cluster/shared
    ```

    如果 `nfsd` 内核模块在宿主机上不可用（某些 Docker Desktop 环境可能缺少），可以用 Compose 中的 volume 直接共享——容器启动后所有节点的 `/cluster/shared` 已经是同一个目录。在报告中如实地说明情况即可。

    参考实现的 Slurm 启动方式与虚拟机不同（没有 systemd），需要在容器内直接运行：
    ...
    使用 `slurm.conf` 时注意将 `ProctrackType` 设为 `proctrack/linuxproc`，详见上面的保底方案说明。

---

## Docker 路线保底方案

如果你在完成本 Lab 的过程中遇到了环境问题，或者希望完全通过 Docker 完成所有任务，这里提供一套"保底"流程。它不是官方路线，但能让你在不出大问题的前提下完整经历 **OpenMPI 编译 → NFS 配置 → Slurm 部署 → HPL 提交** 的完整链条。
...
| 共享文件 | 先用 Docker volume 确保所有容器访问同一路径；再在容器内配置 NFS 将 volume 路径导出给其他容器 | Volume 是宿主机目录直挂；NFS 做了文件系统协议封装，但仍是同一台宿主机的存储 |
...
- 你的 Docker Desktop 环境（Windows / macOS）内核模块不完整，NFS kernel server 无法启动
- 你花了很多时间在 NFS 配置上仍然报 `exportfs: does not support NFS export`，且已确认 `nfsd` 模块无法加载
- 你的宿主机资源有限，不想起太多虚拟机

### 保底做法要点

**NFS 保底：用 Volume + 报告说明替代**

如果确定 NFS 在容器内无法工作，可以在 Compose 文件中使用 named volume 让四个容器共享同一路径（`/cluster/shared`）。在实验报告中如实地说明：

> "由于 Docker Desktop 在 __(Windows/macOS)__ 上运行时，宿主虚拟机未加载 `nfsd` 内核模块，NFS kernel server 无法在容器内正常导出。我使用 Docker named volume 替代 NFS，四个容器将同一个 volume 挂载到 `/cluster/shared`，达到了所有节点访问同一文件的效果。虽然这不是真正的 NFS 协议通信，但共享目录的用途等价。我在下文说明了 NFS 的工作原理和配置命令，并补充了如果在支持 NFS 的环境中如何操作。"

这段话体现了你对 NFS 的理解，也诚实交代了环境限制。

**Slurm 保底：cgroup 退化为 proctrack/linuxproc**
...
无论是否走保底方案，你的实验报告都应包含以下分析，以证明你理解这些环节的原理：
