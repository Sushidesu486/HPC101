...
=== "Docker"
    ...
    !!! warning "Docker 版本的定位"

        这里使用 Docker volume 模拟所有节点都能看到同一路径下的文件，它并不等同于真实的 NFS。当前 Lab 的必做内容仍然包含 NFS 和 Slurm；如果你希望完全用容器完成必做部分，需要额外处理 systemd、cgroup、特权模式和内核 NFS 服务等问题，并在报告中说明你的处理方式。
    ...
    你需要准备一个 `Dockerfile`，用于构建所有节点共用的镜像。镜像中至少需要包含 SSH 服务、编译工具链，以及你在任务一中构建 OpenMPI、BLAS、CBLAS、HPL 所需的环境。下面只是结构示意，请根据自己的任务一过程补全：
    ...
    # TODO: 配置普通用户和 SSH 免密登录
    # TODO: 按任务一要求构建 OpenMPI、BLAS、CBLAS 和 HPL
    # TODO: 准备共享工作目录，例如 /cluster/shared

    CMD ["/usr/sbin/sshd", "-D"]
    ```

    然后使用 `docker-compose.yml` 创建四个服务。重点是让四个容器使用同一个自定义网络，并设置清晰的主机名：
    ...
    实际编写时，你通常还需要补充共享 volume、网络配置、容器名等内容。启动时可以使用：
    ...
    在 Docker Desktop 中，可以截取镜像构建记录、镜像列表、四个容器运行状态等截图。进入 `node01` 后，需要验证主机名解析和 SSH 免密：
    ...
    最后创建 `hostfile`，测试 MPI 是否能在多个节点上启动进程：

    ```bash
    # hostfile 中写入 node01 到 node04
    mpirun --hostfile /cluster/shared/hostfile -np 4 hostname
    ```

    如果你已经把 HPL 放到共享目录中，可以继续运行：

    ```bash
    cd /cluster/shared/hpl-2.3/bin/Linux_PII_FBLAS
    mpirun --hostfile /cluster/shared/hostfile -np 4 ./xhpl | tee /cluster/shared/hpl-docker.out
    ```

    报告中建议记录 Dockerfile / Compose 的关键设计、Docker Desktop 中的构建和容器截图、SSH 验证结果、MPI 连通性测试，以及 HPL 输出结果。注意说明：Docker 路线只是模拟多节点环境，不替代后续真实 NFS 和 Slurm 配置。
